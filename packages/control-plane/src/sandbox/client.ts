...
import type { SessionRepositoryInfo, SandboxStartupContext } from "./provider";
...
const modalErrorResponseSchema = z.object({
  ...
  error_reason: z.enum(["image_unavailable", "unknown"]).optional(),
});
...
export interface CreateSandboxRequest extends SandboxStartupContext {
  sessionId: string;
...
export interface RestoreSandboxRequest extends SandboxStartupContext {
...
export class ModalApiError extends Error {
  constructor(
    ...
    public readonly reason: "image_unavailable" | "unknown" = "unknown"
  ) {
    super(message);
    ...
    try {
      ...
      const response = await fetch(this.createSandboxUrl, {
        ...
        body: JSON.stringify({
          ...
          startup_attempt_id: request.startupAttemptId,
      ...
      if (!result.success) {
        throw new ModalApiError(
          `Modal API error: ${result.error || "Unknown error"}`,
          response.status,
          result.error_reason
        );
      }
